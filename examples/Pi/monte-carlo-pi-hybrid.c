/*
	MPI+OpenMP hybrid twol level parallelism version for PI for acceleration of execution time with the help of parallelism.

	compiler: mpicc -o mpi monte-carlo-pi-mpi.c -fopenmp
  ...
  if(myid != 0 && myid !=size-1)
    {
      ...
      {
      srand48(time(NULL) ^ omp_get_thread_num());//Give rand() a seed value
      if(omp_get_thread_num != 0){
      for (i=0; i<nb/numthreads; ++i)//main loop
	{
      ...
      for (i=0; i<nb/numthreads+nb%numthreads; ++i)//main loop
        {
          x = (double)drand48();//gets a random x coordinate
          y = (double)drand48();//gets a random y coordinate
          z = ((x*x)+(y*y));//Checks to see if number in inside unit circle
  ...
  else if( myid ==size-1)
    {
      ...
      srand48(time(NULL)^omp_get_thread_num());//Give rand() a seed value
      if(omp_get_thread_num != 0){
      for (i=0; i<nb2/numthreads; ++i)//main loop
        {
          x = (double)drand48();//gets a random x coordinate
          y = (double)drand48();//gets a random y coordinate
          z = ((x*x)+(y*y));//Checks to see if number in inside unit circle
      ...
      for (i=0; i<nb2/numthreads+nb2%numthreads; ++i)//main loop
        {
          x = (double)drand48();//gets a random x coordinate
          y = (double)drand48();//gets a random y coordinate
          z = ((x*x)+(y*y));//Checks to see if number in inside unit circle
